...
public class 대출현황표_main {
	...
	public static void main(String[] args) {
		...
		System.out.println(sdf.format(now)); //현재 날짜를 출력해준다
		
	    
		// 동일한 객체를 생성하고 내용만 다를 때 배열을 사용한다.
		대출현황표 d = new 대출현황표("J02-38", "이민주", new Date(), "주택자금대출", 27000000, 48);
		...
		for(int i=0;i<dArr.length;i++) {
			System.out.println(dArr[i].get대출금액() + " " + dArr[i].get성명() + " " + dArr[i].get대출종료() + " " + dArr[i].get대출금액() + " " + dArr[i].get기간());
			sum = sum + dArr[i].get대출금액();
			sum1 = sum1 +dArr[i].get기간();
		}
		
		double avg = sum1/(double)dArr.length;
